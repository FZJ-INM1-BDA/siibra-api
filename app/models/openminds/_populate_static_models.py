...
this script requires datamodel-code-generator==0.13.5, which is not installed via requirements.
Use 0.13.5 to ensure reprod
"""
...
from base import SiibraBaseModel
...
def process_schema(model: OpenmindsSchema):
    ...
    generate(
        ...
        base_class="app.models.openminds.base.SiibraBaseModel"
    )
    with open(output_filename, "r") as fp:
        txt = fp.read()
    with open(output_filename, "w") as fp:
        fp.write(
            txt.replace("https://openminds.ebrains.eu/vocab/", "")
                .replace("HttpsOpenmindsEbrainsEuVocab", "")
                .replace("_type: str = Field(..., alias='@type')", "type: str = Field(..., alias='@type')")
                .replace("_id: str", "id: str")
                .replace("https___openminds_ebrains_eu_vocab_", "")
        )
